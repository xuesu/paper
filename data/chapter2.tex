...
\subsubsection{英文-Stanford CoreNLP工具}
...
Stanford CoreNLP工具是由斯坦福大学自然语言处理实验室研发的\cite{corenlp}，可以处理分句(ssplit)，分词(tokenize)，词性标注(POS)，词根化(lemma)，命名实体分析(NER)，语法树分析(parse)，短句情感分析(sentiment)和词语分析(natlog)等多种任务的工具集。该工具集可以处理多种语言，其中对英文支持最为全面准确。本文中主要用到其中的分词，分句，词性标注，词根化，命名实体分析，语法树分析等功能。\\
...
& 层级 & 中文 & 英文\\
\hline
知识模型 & 语句级 & 词汇(D) & 词根化词汇(D)\\
\hline
CNN模型 & 语句级 & 词汇编号(Emb) & 词汇编号(Emb)，词根编号(Emb)\\
\hline
RNN模型 & 语句级 & 词汇编号(Emb) & 词汇编号(Emb)，词根编号(Emb)\\
\hline
CNN模型 & 实体级 & 词汇编号(Emb)，词性编号(Onehot) & 词汇编号(Emb)，词根编号(Emb)，NER特征编号(Onehot)\\
\hline
RNN模型 & 实体级 & 词汇编号(Emb)，词性编号(Onehot) & 词汇编号(Emb)，词根编号(Emb)，NER特征编号(Onehot)\\
\hline
\end{tabu}
\end{center}
...
\subsection{词根化}
英文中动词往往具有多种时态，如过去式，现在进行时，一般现在时等，如got,gotten,get,getting,gets。同时，形容词和副词也常常具有同样的词根，如happy, happily。这些单词虽然形态不同，却具有语义上的联系，因此，有必要对单词进行词性还原和词干提取，以发现其内部隐含的语义联系。\par
本文使用Stanford CoreNLP对单词进行词性还原，之后再使用nltk Snowball Stemmer进行词根化，以此作为英文模型的输入特征。词根化工具准确率分析见附录。